...
- WordPress テーマ、[BX-05](https://demo.bbns.jp/bx-05/)のリポジトリです
- 当テーマで使用している CSS、JS、画像については[html-bx-05](https://github.com/Yuuji-Hasegawa/html-bx-05)をお使いいただくか、別途用意の上加工してください
...
4. 使用するテーマを bx-05 に変更し、パーマリンクを変更してください
...
1. テーマ名を変更する場合は、`.gitignore`内のテーマ名部分、`wp-bx-05/style.css`を編集してください
2. 各サイトの設計に合わせ、`wp-bx-05/lib/breadcrumb.php`、`wp-bx-05/lib/json-ld.php`を編集ください
3. 固有名詞や favicon、フォントの preload は、`wp-bx-05/lib/setting.json`、、`wp-bx-05/lib/head.php`、`wp-bx-05/pwa/manifest.json`、`wp-bx-05/pwa/sw.js`を編集ください
4. `page-hoge.php`の`hoge`は slug に合わせて編集ください。固定ページの中身を`the_content`で流し込む場合は設定に合わせてください
5. jQuery や reCAPTCHA の読み込みを`wp-bx-05/lib/script-load.php`で制御しています。適宜編集ください
